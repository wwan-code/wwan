...
const RatingDisplay = ({ slug, data }) => {
    const [ratingCount, setRatingCount] = useState(null);
    const [loading, setLoading] = useState(true);
    const [rating, setRating] = useState(0);
    const [hover, setHover] = useState(0);
    const rate = ratingCount ? Math.floor(ratingCount) : 0;
    ...
    useEffect(() => {
        const fetchRating = async () => {
            try {
                const response = await axios.get(`/api/rating/${slug}`, { headers: authHeader() });
                setRatingCount(response.data.averageRating);
            } catch (error) {
                toast.error("Lỗi khi tải rating.", {
    ...
    return (
        <div className="movie-single__rate">
            <div className="movie-single__rate-rate">
                ...
                <p><span>{ratingCount || 0}</span> /10<br />
                    <span className="rv">56 Reviews</span>
                </p>
            </div>
            <div className="movie-single__rate-star">
                <p>Đánh giá:</p>
                    ...
                    {[...Array(10)].map((_, index) => {
                        ...
                        return (
                            <svg
                                ...
                                viewBox="0 0 24 24"
                                fill="currentColor"
                                className={`star ${starValue <= (hover || rating || rate) ? "filled" : "empty"}`}
