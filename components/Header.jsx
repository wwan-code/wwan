...
import NotificationSwipeableItem from './Notification/NotificationSwipeableItem';
...
const Header = () => {
    ...
    const handleUserDropdownToggle = (e) => {
        ...
        // Nếu dropdown sắp đóng, đảm bảo panel theme cũng đóng
        if (openDropdown === USER_DROPDOWN_ID) {
            setShowThemeSettingsPanel(false);
        }
    };
    ...
    const handleThemeSettingToggle = (e) => {
        e.stopPropagation(); // Ngăn dropdown chính đóng lại
    ...
    const loadMoreNotifications = () => {
        if (notificationPagination.currentPage < notificationPagination.totalPages && !loadingMore) {
    ...
    const handleDailyCheckIn = async () => {
        ...
        try {
            const response = await axios.post('/api/gamification/daily-check-in', {}, { headers: authHeader() });
    ...
    return (
        <header className={classNames("header", { "header-fixed": isScroll && !isOnComicReaderPage })}>
                    ...
                    <div className="header-menu">
                        <nav className="navbar">
                            <ul className='navbar-nav'>
                                ...
                                <li className='nav-item'>
                                    <OverlayTrigger placement="bottom" overlay={renderTooltip({ children: "Truyện Tranh" })}>
                                        <NavLink className={({ isActive, isPending }) =>
                                            classNames("nav-link", { "active": isActive, "pending": isPending })
                                        } to={'/truyen-tranh'}>
                    ...
                    <div className="header-right">
                        ...
                        {currentUser && (
                            <div className="dropdown-notification dropdown" ref={(el) => dropdownRefCallback(el, NOTIFICATION_DROPDOWN_ID)}>
                                ...
                                <div
                                    ...
                                    data-bs-popper
                                >
                                    <div className="dropdown-notification-header border-bottom">
                                    ...
                                    <div className="dropdown-notification-body overflow-auto py-1" style={{ maxHeight: '350px' }}>
                                        ...
                                        {!loadingNotifications && notificationPagination.currentPage < notificationPagination.totalPages && (
                                            <div className="text-center py-2 border-top">
                                                <button className="btn btn-sm btn-link" onClick={loadMoreNotifications} disabled={loadingMore}>
                                                    {loadingMore ? <span className="spinner-border spinner-border-sm"></span> : "Xem thêm"}
                                                </button>
                                            </div>
                                        )}
                                    </div>
                        ...
                        {isLoggedIn && (
                            <OverlayTrigger
                                placement="bottom"
                                overlay={
                                    <Tooltip id="tooltip-daily-checkin">
                                        {canCheckInToday ? `Điểm danh nhận 15 điểm!` : "Hôm nay bạn đã điểm danh"}
                                    </Tooltip>
                                }
                            >
                                <button
                                    className={`btn-daily-checkin ${canCheckInToday ? '' : 'checked'}`}
                                ...
                                >
                                    {isCheckingIn ? (
                                        ...
                                        <><i className={`fas ${canCheckInToday ? 'fa-calendar-pen' : 'fa-calendar-check'}`}></i></>
                                    )}
                                </button>
                            </OverlayTrigger>
                        )}
                        {currentUser ? (
                            <div className="dropdown-user dropdown" ref={(el) => dropdownRefCallback(el, USER_DROPDOWN_ID)}>
                                <span role='button' className="dropdown-toggle hide-arrow p-0 d-flex align-items-center justify-content-center" onClick={handleUserDropdownToggle} >
                                    <div className="avatar"><img src={getAvatarUrl()} alt={currentUser.name} className="rounded-circle" /></div>
                                </span>
                                <div
