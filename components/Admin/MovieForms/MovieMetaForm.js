...
const MovieMetaForm = ({
    ...
    countries = [],
    categories = [],
    selectedGenres = [], // Nhận state genre đã chọn
    onSelectedGenresChange, // Nhận hàm set state genre
    onShowAddItemOffcanvas, // Nhận hàm mở Offcanvas
}) => {
    // --- State và Ref cho Multi-Select (Chuyển từ cha vào đây) ---
    ...
    // Lọc genres khi searchTerm hoặc genres gốc thay đổi
    useEffect(() => {
        if (searchTerm === '') {
     ...
     // Reset filteredGenres khi genres gốc thay đổi (ví dụ sau khi thêm mới từ offcanvas)
    useEffect(() => {
        setFilteredGenres(genres);
    }, [genres]);
    ...
    const handleOptionClick = useCallback((genre) => {
        ...
        onSelectedGenresChange(newSelectedGenres); // Gọi hàm của cha để cập nhật state
    }, [selectedGenres, onSelectedGenresChange]);

    const handleRemoveItem = useCallback((id) => {
        const newSelectedGenres = selectedGenres.filter((genre) => genre.id !== id);
        onSelectedGenresChange(newSelectedGenres); // Gọi hàm của cha
    }, [selectedGenres, onSelectedGenresChange]);
        ...
        <>
            ...
            <div className="card mb-4">
                ...
                <div className="card-body">
                    {/* --- Multi-select Genres --- */}
                    <div className="mb-3">
                        <label htmlFor="genres-select" className="d-flex flex-wrap align-items-center mb-2">
                            <h6 className="form-label mb-0 me-2">Thể loại <span className="text-danger">*</span></h6>
                        ...
                        <div id="genres-select" className='form-floating form-floating-advance-select'>
                             <div ref={dropdownRef} className={`form-multi-select ${isOpenSelect ? 'show' : ''}`} data-type="select-multiple">
                                <div className="form-multi-select-input-group" onClick={handleInnerClick}>
                                    {/* Input ẩn không cần thiết nếu không dùng thư viện choices.js */}
                    ...
                    {/* --- Category and Country Selects --- */}
                    <div className="row">
                        <div className="mb-3 col-6">
                            <label className="d-flex flex-wrap align-items-center mb-2">
                                <h6 className="form-label mb-0 me-2">Danh mục <span className="text-danger">*</span></h6>
                        ...
                        <div className="mb-3 col-6">
                            <div className="d-flex flex-wrap align-items-center mb-2">
                                <h6 className="form-label mb-0 me-2">Quốc gia <span className="text-danger">*</span></h6>
                        ...
                        <div className="mb-3 col-6">
                            <label htmlFor="premiere" className="form-label">Ngày công chiếu</label>
                            <input
                                id="premiere"
                                type="date"
                                name="premiere"
                ...
                <div className="card-body">
                     <div className="row">
                         <div className="mb-3 col-md-6">
                            ...
                            <input
                                type="text" // Hoặc text tùy định dạng
                                name="duration"
                                className="form-control"
                                value={data.duration}
                                onChange={onInputChange}
