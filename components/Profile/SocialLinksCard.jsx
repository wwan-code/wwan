...
const SocialLinksCard = ({ socialLinks, isEditing, onSocialLinkChange }) => {
    const links = socialLinks || {}; // Đảm bảo links là object

    // Helper để render từng dòng link
    const renderLinkItem = (key, label, iconClass, colorClass = '') => {
        const value = links[key] || '';
        if (!isEditing && !value) return null; // Không hiển thị nếu không edit và không có link

        return (
            <li key={key} className="list-group-item d-flex justify-content-between align-items-center flex-wrap px-3 py-2">
                ...
                {isEditing ? (
                    <input
                        type="text"
                        name={key} // name trùng với key trong socialLinks
                        className="form-control form-control-sm w-auto flex-grow-1 ms-2" // Style input
                        value={value}
    ...
    return (
        <div className="card mt-3">
            {/* Không cần Card Header nếu không muốn */}
