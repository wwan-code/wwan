...
import useTableData from '../../hooks/useTableData'; // Sử dụng lại hook này

const ContentReportsPage = () => {
    const [reports, setReports] = useState([]);
    const [isLoading, setIsLoading] = useState(true); // Đổi tên từ loading
    ...
    const {
        ...
        totalPages,
        currentPage,
        searchTerm, // Nếu muốn thêm tìm kiếm
        handleSearch, // Nếu muốn thêm tìm kiếm
        requestSort,
        goToPage,
        sortConfig, // Lưu trữ cột và hướng sắp xếp
        itemsPerPage,
        handleItemsPerPageChange,
        totalEntries,
        // ... (các giá trị khác từ useTableData)
    } = useTableData(reports, 20); // 20 items per page

    // State cho filter status (nếu muốn thêm vào useTableData hoặc quản lý riêng)
    const [filterStatus, setFilterStatus] = useState(''); // pending, resolved, etc.
    ...
    const fetchReports = useCallback(async (page = 1, limit = 20, status = '', sortBy = 'createdAt', sortOrder = 'DESC') => {
        ...
        setError(null);
        try {
            const response = await axios.get('/api/admin/content-reports', {
                headers: authHeader(),
                params: { page, limit, status, sortBy, sortOrder }
            });
            if (response.data?.success) {
                setReports(response.data.reports || []); // Cập nhật state gốc cho useTableData
                // useTableData sẽ tự cập nhật pagination dựa trên `reports`
            } else {
    ...
    }, []); // Không có dependency thay đổi thường xuyên
    ...
    const handleUpdateReport = async (e) => {
        ...
        try {
            await axios.put(`/api/admin/content-reports/${currentReport.id}/status`,
                { status: newStatus, adminNotes },
                { headers: authHeader() }
            );
            toast.success("Cập nhật báo cáo thành công!");
            setShowUpdateModal(false);
            fetchReports(currentPage, itemsPerPage, filterStatus, sortConfig.key, sortConfig.direction); // Tải lại
        } catch (error) {
    ...
    const handleDeleteReport = async (reportId) => {
        ...
        try {
            await axios.delete(`/api/admin/content-reports/${reportId}`, { headers: authHeader() });
    ...
    return (
        <div className="container-fluid flex-grow-1 container-p-y">
            <h4 className="mb-4">Quản lý Báo cáo Lỗi Nội dung</h4>
            ...
            {/* Bộ lọc */}
             <Form className="mb-3 d-flex justify-content-end">
                <Form.Group controlId="filterStatus" className="me-2" style={{maxWidth: '200px'}}>
                 ...
                 {/* Có thể thêm input tìm kiếm nếu useTableData hỗ trợ */}
            </Form>
