...
// SCSS cho trang này và trang quản lý chapter có thể dùng chung một phần
...
const DraggableExistingPageItem = ({ page, index, movePage, onRemove, isSubmitting }) => {
    ...
    const imageUrl = page.imageUrl.startsWith('http') ? page.imageUrl : `/uploads/${page.imageUrl}`;
...
const ComicPageManagement = () => {
    ...
    const [newImageFiles, setNewImageFiles] = useState([]); // Files mới để upload
    const [newImagePreviews, setNewImagePreviews] = useState([]); // Previews cho files mới
    ...
    const fetchChapterWithPages = useCallback(async () => {
        ...
        try {
            const response = await axios.get(`/api/chapters/${chapterId}/pages`, { headers: authHeader() });
            if (response.data?.success && response.data.chapter) {
                setChapter(response.data.chapter);
                // Sắp xếp pages theo pageNumber trước khi set
    ...
    const moveExistingPage = useCallback((dragIndex, hoverIndex) => {
        ...
        // Sau khi kéo thả, pageNumber sẽ không còn đúng nữa, cần cập nhật lại khi lưu
    }, []);

    const handleRemoveExistingPage = async (pageIdToRemove, indexInUI) => {
        // Hỏi xác nhận trước khi xóa thật khỏi DB
        if (!window.confirm(`Bạn chắc chắn muốn xóa trang ảnh này (ID: ${pageIdToRemove}) vĩnh viễn?`)) return;
        setIsSubmitting(true);
        try {
            await axios.delete(`/api/admin/comic-pages/${pageIdToRemove}`, { headers: authHeader() });
    ...
    const handleSaveChanges = async () => {
        ...
        try {
            // API này cần được tạo ở backend để xử lý cả việc sắp xếp lại và thêm mới
            // PUT /api/admin/chapters/:chapterId/manage-pages
            const response = await axios.put(
                `/api/admin/chapters/${chapterId}/manage-pages`,
                formData,
                { headers: { ...authHeader() } }
            );
            ...
            if (response.data?.success) {
                ...
                setNewImageFiles([]); // Reset file mới
                setNewImagePreviews([]);
                // Tải lại toàn bộ trang và chương để có thứ tự và pageNumber đúng nhất từ server
                fetchChapterWithPages();
            } else {
