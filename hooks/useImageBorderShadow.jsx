...
const MAX_CANVAS_DIMENSION = 100; // Giới hạn kích thước canvas để xử lý nhanh hơn
const SAMPLE_AREA_SIZE = 5;     // Kích thước vùng lấy mẫu (nhỏ hơn để nhanh hơn)
...
const useImageBorderShadow = (imageUrl) => {
    ...
    useEffect(() => {
        ...
        const img = new Image(); // Tạo Image object mới để không phụ thuộc vào imgRef đã render
        img.crossOrigin = "Anonymous";
        // Kiểm tra imageURL co thư mục uploads hay khong, neu khong thi them
        ...
        if (
            ...
            finalImageUrl = `uploads/${imageUrl.replace(/^\/+/, '')}`;
        }
        img.src = finalImageUrl.startsWith('http') ? finalImageUrl : `/${finalImageUrl}`;
        ...
        // Cleanup: Không cần vì chúng ta tạo Image object mới mỗi lần
        // return () => { img.onload = null; img.onerror = null; }

    }, [imageUrl, getAverageColorFromContext]); // Thêm getAverageColorFromContext vào dependencies

    // imgRef vẫn được trả về để component cha có thể gán vào thẻ <img> nếu muốn
    // Tuy nhiên, logic tính toán màu giờ đây độc lập với imgRef này.
